...
export default function MonthPage() {
    ...
    useEffect(() => {
        ...
        fetch('/api/month')
            .then(res => res.json())
            .then(data => setDbMonth(data))
            .finally(() => setLoading(false));
    }, []);

    const chartOption = useMemo(() => {
        const measurements = dbMonth?.measurements?.[interval] ?? {};
        const forecastData = dbMonth?.forecast ?? [];
    ...
    }, [dbMonth, interval, mobileDevice, darkMode]);

    const summary = dbMonth?.summary?.[interval] ?? {};

    const timestamps = Object.keys(dbMonth?.measurements?.[interval] ?? {});
