...
export default function GroupedYearPage() {
    const {
        darkMode,
        setDarkMode,
        mobileDevice,
    ...
    const chartOption = useMemo(() => {
        ...
        const forecastData = dbGroupedYear?.forecast ?? {};
        ...
        const monthBreakMarkLines = uniqueMonths.slice(1).map(month => {
            ...
            return {
                name: `${MONTH_NAMES_GY[parseInt(mon, 10) - 1]} ${year}`,
                xAxis: ts,
    ...
    }, [dbGroupedYear, interval, mobileDevice, darkMode]);

    const summary = dbGroupedYear?.summary?.[interval] ?? {};

    const timestamps = Object.keys(dbGroupedYear?.measurements ?? {}).sort();
