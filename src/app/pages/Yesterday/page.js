...
export default function YesterdayPage() {
    ...
    const chartOption = useMemo(() => {
        const measurements = dbYesterday?.measurements?.[interval] ?? {};
        const forecastData = dbYesterday?.forecast ?? [];
        ...
        // Hour-break vertical lines at the start of each hour after the first
        const hourBreakMarkLines = timestamps
            .filter(dt => dt.substring(14, 16) === '00' && dt !== timestamps[0])
            .map(ts => {
        ...
        return {
            ...
            dataZoom: [
                ...
                { type: 'slider', start: 0, end: 80, textStyle: { color: darkMode ? '#9ca3af' : '#666' }, borderColor: darkMode ? '#374151' : '#ccc', fillerColor: darkMode ? 'rgba(55,65,81,0.5)' : 'rgba(0,139,193,0.1)', backgroundColor: darkMode ? '#1f2937' : '#f8f8f8' },
            ],
            series: [
                {
                    name: 'PTA',
    ...
    }, [dbYesterday, interval, mobileDevice, darkMode]);

    const summary = dbYesterday?.summary?.[interval] ?? {};

    const timestamps = Object.keys(dbYesterday?.measurements?.[interval] ?? {}).sort();
    const weekLabel = timestamps.length
    ...
    return (
        <Layout>
            <div style={{ padding: '24px 24px 48px 24px', overflowY: 'auto', height: '100%', background: darkMode ? '#111827' : undefined }}>
                ...
                {/* {weekLabel && (
                    <h2 style={{ margin: '0 0 20px 0', fontSize: 20, fontWeight: 700, color: '#008bc1' }}>
                        {weekLabel}
                    </h2>
                )} */}
                ...
                <div style={{
                    ...
                    <div style={{ touchAction: 'none', position: 'relative' }}>
                        <div style={{ position: 'absolute', top: 0, left: 0, zIndex: 10, display: 'flex', gap: 4 }}>
                            {INTERVALS.map(iv => (
                                <button
                                    key={iv}
