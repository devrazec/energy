...
export default function GroupedDayPage() {
    const {
        darkMode,
        setDarkMode,
        mobileDevice,
    ...
    const chartOption = useMemo(() => {
        const measurements = dbGroupedDay?.measurements ?? {};
        const forecastData = dbGroupedDay?.forecast ?? {};
    ...
    }, [dbGroupedDay, interval, mobileDevice, darkMode]);

    const summary = dbGroupedDay?.summary?.[interval] ?? {};

    const timestamps = Object.keys(dbGroupedDay?.measurements ?? {}).sort();
