...
export default function WeekPage() {
    ...
    const chartOption = useMemo(() => {
        const measurements = dbWeek?.measurements?.[interval] ?? {};
        const forecastData = dbWeek?.forecast ?? [];
    ...
    }, [dbWeek, interval, mobileDevice, darkMode]);

    const summary = dbWeek?.summary?.[interval] ?? {};

    const timestamps = Object.keys(dbWeek?.measurements?.[interval] ?? {}).sort();
