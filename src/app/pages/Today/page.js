...
export default function TodayPage() {
    ...
    const chartOption = useMemo(() => {
        const measurements = dbToday?.measurements?.[interval] ?? {};
        const forecastData = dbToday?.forecast ?? [];
    ...
    }, [dbToday, interval, mobileDevice, darkMode]);

    const summary = dbToday?.summary?.[interval] ?? {};

    const timestamps = Object.keys(dbToday?.measurements?.[interval] ?? {}).sort();
