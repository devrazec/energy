...
export default function TomorrowPage() {
    ...
    const chartOption = useMemo(() => {
        ...
        const forecastData = dbTomorrow?.forecast ?? [];
        ...
        const forecastSeries = forecastData.map(f => [f.datetime, f.active_power]);
        ...
        return {
            ...
            legend: { data: ['Forecast'], orient: 'vertical', top: 'middle', right: 0, textStyle: { color: darkMode ? '#d1d5db' : '#333' } },
            grid: isMobile
            ...
            dataZoom: [
                ...
                { type: 'slider', start: 0, end: 50, textStyle: { color: darkMode ? '#9ca3af' : '#666' }, borderColor: darkMode ? '#374151' : '#ccc', fillerColor: darkMode ? 'rgba(55,65,81,0.5)' : 'rgba(0,139,193,0.1)', backgroundColor: darkMode ? '#1f2937' : '#f8f8f8' },
            ],
            series: [
                {
                    name: 'PTA',
    ...
    }, [dbTomorrow, interval, mobileDevice, darkMode]);

    const summary = dbTomorrow?.summary?.[interval] ?? {};

    const forecastData = dbTomorrow?.forecast ?? [];
    const forecastTotalEnergy = forecastData.reduce((sum, f) => sum + (f.active_power ?? 0) * 0.25, 0);

    const allTimestamps = Object.keys(dbTomorrow?.measurements?.[interval] ?? {}).sort();
    const forecastTimestamps = (dbTomorrow?.forecast ?? []).map(f => f.datetime).sort();
    const firstTs = allTimestamps[0] ?? forecastTimestamps[0];
    const dateLabel = firstTs ? firstTs.substring(0, 10) : null;
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                                ...
                                <div style={{ textAlign: 'right' }}>
                                    ...
                                    <div style={{ fontSize: 22, fontWeight: 800, color: textPrimary, lineHeight: 1 }}>
                                        {(data?.total_active_energy ?? forecastTotalEnergy).toFixed(2)}
