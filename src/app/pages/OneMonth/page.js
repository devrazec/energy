...
export default function OneMonthPage() {
    ...
    const chartOption = useMemo(() => {
        const measurements = dbOneMonth?.measurements?.[interval] ?? {};
        const forecastData = dbOneMonth?.forecast ?? [];
        ...
        const forecastSeries = timestamps.map(dt => [dt, forecastMap[dt] ?? null]);

        // Day-break vertical lines at midnight of each unique date after the first
        const uniqueDates = [...new Set(timestamps.map(dt => dt.substring(0, 10)))];
        const dayBreakMarkLines = uniqueDates.slice(1).map(date => {
            const ts = timestamps.find(dt => dt.startsWith(date));
            if (!ts) return null;
            const dayName = DAY_NAMES[new Date(date + 'T12:00').getDay()];
            return {
                name: `${dayName} ${date.substring(5).replace('-', '/')}`,
                xAxis: ts,
        ...
        return {
            ...
            grid: isMobile
                ? { left: 10, right: 10, bottom: 100, top: 56, containLabel: true }
            ...
            dataZoom: [
                ...
                { type: 'slider', start: 0, end: 20, textStyle: { color: darkMode ? '#9ca3af' : '#666' }, borderColor: darkMode ? '#374151' : '#ccc', fillerColor: darkMode ? 'rgba(55,65,81,0.5)' : 'rgba(0,139,193,0.1)', backgroundColor: darkMode ? '#1f2937' : '#f8f8f8' },
            ],
            series: [
                {
                    name: 'PTA',
    ...
    }, [dbOneMonth, interval, mobileDevice, darkMode]);

    const summary = dbOneMonth?.summary?.[interval] ?? {};

    const timestamps = Object.keys(dbOneMonth?.measurements?.[interval] ?? {});
    const firstTs = timestamps.sort()[0];
    const monthYear = firstTs
        ? `${MONTHS[parseInt(firstTs.substring(5, 7), 10) - 1]} ${firstTs.substring(0, 4)}`
        : null;
    ...
    const btnInactiveBg = darkMode ? '#374151' : '#fff';
    const btnInactiveBorder = darkMode ? '#4b5563' : '#ccc';
    const btnInactiveColor = darkMode ? '#d1d5db' : '#333';
            ...
            <div style={{ padding: '24px 24px 48px 24px', overflowY: 'auto', height: '100%', background: darkMode ? '#111827' : undefined }}>
                ...
                <div style={{
                    ...
                    <div style={{ touchAction: 'none', position: 'relative' }}>
                        <div style={{ position: 'absolute', top: 4, left: 0, zIndex: 10, display: 'flex', gap: 4 }}>
                            {INTERVALS.map(iv => (
                                <button
                                    key={iv}
                                    ...
                                    style={{
                                        padding: '2px 10px',
                                        borderRadius: 5,
                                        border: `1px solid ${interval === iv ? '#008bc1' : btnInactiveBorder}`,
                                        cursor: 'pointer',
                                        fontSize: 12,
                                        fontWeight: interval === iv ? 'bold' : 'normal',
                                        background: interval === iv ? '#008bc1' : btnInactiveBg,
                                        color: interval === iv ? '#fff' : btnInactiveColor,
                                    }}
                                >
                                    {iv}
                                </button>
