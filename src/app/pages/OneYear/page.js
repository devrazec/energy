...
export default function OneYearPage() {
    ...
    const chartOption = useMemo(() => {
        const measurements = dbOneYear?.measurements?.[interval] ?? {};
        const forecastData = dbOneYear?.forecast ?? [];
        ...
        return {
            ...
            dataZoom: [
                ...
                { type: 'slider', start: 0, end: 10, textStyle: { color: darkMode ? '#9ca3af' : '#666' }, borderColor: darkMode ? '#374151' : '#ccc', fillerColor: darkMode ? 'rgba(55,65,81,0.5)' : 'rgba(0,139,193,0.1)', backgroundColor: darkMode ? '#1f2937' : '#f8f8f8' },
            ],
            series: [
                {
                    name: 'Production',
    ...
    }, [dbOneYear, interval, mobileDevice, darkMode]);

    const summary = dbOneYear?.summary?.[interval] ?? {};

    const timestamps = Object.keys(dbOneYear?.measurements?.[interval] ?? {});
